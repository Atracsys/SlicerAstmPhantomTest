...
class AstmPhantomTestLogic(ScriptedLoadableModuleLogic, vtk.vtkObject):
  ...
  def EndSession(self):
    ...
    f.write(
      ...
      f'For accuracy, each measurement is treated as a vector from the corresponding reference point (central divot). The accuracy mean is the length of the average of these vectors. The accuracy max is the length of the longest vector.<br>\n'
      f'For precision, the deviations are calculated as the distances of all the measurements from their average. The Root Mean Square (RMS) and the maximum of these distances are reported.\n'
      f'<p>\n'
      ...
      f'  <tr><td>Std</td><td><b>{sv[4][0]}</td><td>{sv[4][1]}</td><td>{sv[4][2]}</td><td>{sv[4][3]}</td><td>{sv[4][4]}</td><td>{sv[4][5]}</td></tr>\n'
      f'</table>\n'
      f'\n'
      f'<h3>Rotation Precision Tests</h3>\n'
      f'The rotation tests measures the precision of single point acquisition under various orientations of the pointer. These orientations consists of <b>successive</b> rotations around the roll, pitch and yaw axes of the pointer.<br>\n'
      ...
      f'For each rotation axis (roll, pitch, yaw), the minimum and maximum angles are reported, as well as the span (largest difference between two measurements) and the RMS of all measurements.\n'
      f'\n'
      ...
      f'This test measure the spatial relationship between various acquired points and compare it to the reference values. This can be done in two ways: distances or registration.<br>\n'
      f'The distances are calculated for all combinations of pair of points. So, for N points measured, there are N!/(2*(N-2)!) distances (e.g. 20 measured points gives 190 distances). For each pair of points, the error is calculated as the difference with the corresponding reference pair in terms of distance. The number of distances and the mean, minimum, maximum, standard deviation and RMS of these errors are reported below.<br>\n'
